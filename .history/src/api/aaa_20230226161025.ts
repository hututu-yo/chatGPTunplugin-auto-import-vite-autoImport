...
const axiosInstance: <AxiosStatic| undefined>  = axios.create({
  timeout: 60000,
});
...
const afterResponse = (response: AxiosResponse): Response => {
  console.log("response: ", response);
  if (loadingInstance) {
    loadingInstance.close();
  }
  ...
  // const NewResponse = { ...response, code: 200 };
  return { ...response, code: 200 };
  // return NewResponse;
};
